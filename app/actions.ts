...
export async function startRender(form: FormData) { const contentId = val(form, "contentId"); let target:string;try { const renderId = await submitRender(contentId);target=`/studio?content=${encodeURIComponent(contentId)}&render=${encodeURIComponent(renderId)}`; } catch (error) { target=`/studio?content=${encodeURIComponent(contentId)}&renderError=${encodeURIComponent(safeErrorCode(error))}`; }redirect(target); }
