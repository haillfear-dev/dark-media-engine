...
export async function selectCandidate(form: FormData) { const ideaId = val(form, "ideaId"); db().prepare("UPDATE ideas SET status='SELECTED' WHERE id=?").run(ideaId); redirect(`/studio?idea=${ideaId}`); }
...
export async function feedback(form: FormData) { const status = val(form, "action") === "approved" ? "APPROVED" : val(form, "action") === "rejected" ? "REJECTED" : "DRAFT"; db().prepare("UPDATE contents SET status=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").run(status, val(form, "contentId")); db().prepare("INSERT INTO editorial_feedback(id,content_id,action,reason,note) VALUES(?,?,?,?,?)").run(id("feedback"), val(form, "contentId"), val(form, "action"), val(form, "reason") || null, val(form, "note") || null); revalidatePath("/studio"); }
