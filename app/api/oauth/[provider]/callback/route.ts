...
export async function GET(request: NextRequest, { params }: { params: Promise<{ provider: string }> }) {
  ...
  try { const brandId = consumeOAuthState(provider, state); await finishOAuth(provider, brandId, code); const response = NextResponse.redirect(new URL(`/integrations?connected=${provider}`, request.url)); response.cookies.set("oauth_state", "", { httpOnly: true, sameSite: "lax", maxAge: 0, path: "/api/oauth" }); return response; }
