...
test("TikTok Desktop PKCE uses a cryptographically random verifier and a lowercase hexadecimal S256 challenge", () => { const first = randomCodeVerifier(), second = randomCodeVerifier(); assert.notEqual(first, second); assert.match(first, /^[A-Za-z0-9_-]{43,128}$/); const expected = createHash("sha256").update(first).digest("hex"); assert.equal(codeChallenge(first), expected); assert.match(codeChallenge(first), /^[0-9a-f]{64}$/); });
