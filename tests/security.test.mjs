...
test("TikTok Desktop PKCE uses a random verifier and a lowercase SHA-256 hex challenge", () => { const first = randomCodeVerifier(), second = randomCodeVerifier(); assert.notEqual(first, second); assert.match(first, /^[A-Za-z0-9_-]{43,128}$/); assert.equal(codeChallenge(first), createHash("sha256").update(first).digest("hex")); assert.match(codeChallenge(first), /^[a-f0-9]{64}$/); });
