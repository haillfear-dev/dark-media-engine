...
import { tiktokAuthorizationUrl, tiktokTokenBody } from "../src/social/tiktok-oauth.ts";
...
test("TikTok token exchange body carries the original PKCE verifier", () => { const body = tiktokTokenBody(tiktokConfiguration, "authorization-code", "original-verifier"); assert.equal(body.get("code_verifier"), "original-verifier"); assert.equal(body.get("code"), "authorization-code"); assert.equal(body.get("redirect_uri"), tiktokConfiguration.redirectUri); });
